...
mean_week1_sleep <- mean(week1_sleep)
print(mean_week1_sleep)
...
over_8_hours_week1 <- sum(week1_sleep > 8)
print(over_8_hours_week1)
...
total_points_assignments <- colSums(grades)
print(total_points_assignments)

# Total points for each student using `rowSums()`
total_points_students <- rowSums(grades)
print(total_points_students)
...
factor_recommendations_vector <- factor(recommendations_vector, ordered = TRUE, levels = c("no", "neutral", "yes"))

# Use the `summary()` function to print a summary of `factor_recommendations_vector`
summary(factor_recommendations_vector)

# Using the built-in `mtcars` data set, view the first few rows using the `head()` function
head(mtcars)

# Using the built-in mtcars data set, view the last few rows using the `tail()` function
tail(mtcars)
